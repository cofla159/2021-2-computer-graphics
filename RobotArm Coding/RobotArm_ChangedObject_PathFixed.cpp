...
int RightButtonDown = 0;    //마우스 오른쪽버튼 변수 추가
int RobotControl = 0;
...
const char* ourObjectPath = "C:/Users/Visitor/source/repos/cofla159/LearnOpenGL/homework3__RobotArm.dir/Debug./et.obj";
// translate it so it's at the center of the scene
// it's a bit too big for our scene, so scale it down
glm::mat4 objectXform = glm::rotate(glm::scale(glm::translate(glm::mat4(1.0f), glm::vec3(0.7f, 0.04f, 0.0f)), glm::vec3(0.0008f, 0.0008f, 0.0008f)), glm::radians(-90.0f), glm::vec3(1.0, 0.0, 0.0));
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS)      //W/S/A/D가 눌렸을 때 카메라가 deltaTime만큼 각 방향으로 움직이도록 함
		camera.ProcessKeyboard(FORWARD, 30 * deltaTime);   //(원활한 이동을 위해 30배씩 함)
	if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS)
		camera.ProcessKeyboard(BACKWARD, 30 * deltaTime);
	if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
		camera.ProcessKeyboard(LEFT, 30 * deltaTime);
	if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS)
		camera.ProcessKeyboard(RIGHT, 30 * deltaTime);
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (RightButtonDown)  //오른쪽 버튼이 눌리면 마우스의 xoffset과 yoffset만큼 시선이 움직임(원활한 움직임을 위해 100배 함)
	{
		camera.ProcessMouseMovement(100 * xoffset, 100 * yoffset); 
	}
}
...
void mouse_button_callback(GLFWwindow* window, int button, int action, int mods)
{
	...
	if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_RIGHT) == GLFW_PRESS)  //마우스 오른쪽 버튼이 눌릴때마다 변수가 토글됨
	{
		RightButtonDown = 1;
	}
	else
