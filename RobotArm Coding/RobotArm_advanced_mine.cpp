...
//glm::mat4 objectXform = glm::scale(glm::translate(glm::mat4(1.0f), glm::vec3(0.5f, 0.0f, 0.0f)), glm::vec3(0.08f, 0.08f, 0.08f)); //주전자의 위치가 로봇팔의 움직임에 따라 변화해야 하므로 삭제
...
void myDisplay()
{
    ...
    objectXform = glm::translate(objectXform, glm::vec3(BaseTransX, 0.0f, BaseTransZ)); //teapot의 회전축이 로봇팔을 따라 같이 움직이도록 함
    objectXform = glm::rotate(objectXform, TeapotBaseSpin * 0.01f, glm::vec3(0.0f, 1.0f, 0.0f)); //로봇팔이 회전할 때 teapot도 회전하도록 함
    objectXform = glm::translate(objectXform, glm::vec3(TeapotTransX - BaseTransX - 0.5f, 0.0f, TeapotTransZ - BaseTransZ)); 
    ...
    //Shoulder 움직일 때 teapot
    objectXform = glm::translate(objectXform, glm::vec3(0.0f, 0.4f, 0.0f));
    objectXform = glm::rotate(objectXform, ShoulderAng * 0.02f, glm::vec3(0.0f, 0.0f, 1.0f));
    objectXform = glm::rotate(objectXform, TeapotShoulderAng * 0.02f, glm::vec3(0.0f, 0.0f, 1.0f));
    objectXform = glm::rotate(objectXform, ShoulderAng * 0.02f, glm::vec3(0.0f, 0.0f, -1.0f));
    objectXform = glm::translate(objectXform, glm::vec3(0.0f, -0.4f, 0.0f));
    ...
    //Elbow 움직일 때 teapot
    objectXform = glm::translate(objectXform, glm::vec3(0.0f, 0.9f, 0.0f));
    objectXform = glm::rotate(objectXform, ShoulderAng * 0.02f, glm::vec3(0.0f, 0.0f, 1.0f));
    objectXform = glm::rotate(objectXform, ElbowAng * 0.018f, glm::vec3(0.0f, 0.0f, 1.0f));
    objectXform = glm::rotate(objectXform, TeapotElbowAng * 0.018f, glm::vec3(0.0f, 0.0f, 1.0f));
    objectXform = glm::rotate(objectXform, ElbowAng * 0.018f, glm::vec3(0.0f, 0.0f, -1.0f));
    objectXform = glm::rotate(objectXform, ShoulderAng * 0.02f, glm::vec3(0.0f, 0.0f, -1.0f));
    objectXform = glm::translate(objectXform, glm::vec3(0.0f, -0.9f, 0.0f));
    ...
    objectXform = glm::rotate(objectXform, WristAng * 0.018f, glm::vec3(0.0f, 0.0f, 1.0f));
    objectXform = glm::rotate(objectXform, TeapotWristAng * 0.018f, glm::vec3(0.0f, 0.0f, 1.0f));
    objectXform = glm::rotate(objectXform, WristAng * 0.018f, glm::vec3(0.0f, 0.0f, -1.0f));
    objectXform = glm::translate(objectXform, glm::vec3(0.0f, -0.5f, 0.0f));
    objectXform = glm::rotate(objectXform, ElbowAng * 0.018f, glm::vec3(0.0f, 0.0f, -1.0f));
    objectXform = glm::rotate(objectXform, ShoulderAng * 0.02f, glm::vec3(0.0f, 0.0f, -1.0f));
    ...
    objectXform = glm::translate(objectXform, glm::vec3(0.0f, 0.5f, 0.0f));
    objectXform = glm::rotate(objectXform, WristAng * 0.018f, glm::vec3(0.0f, 0.0f, 1.0f));
    objectXform = glm::rotate(objectXform, WristTwistAng * 0.018f, glm::vec3(0.0f, 1.0f, 0.0f));
    objectXform = glm::rotate(objectXform, TeapotWristTwistAng * 0.018f, glm::vec3(0.0f, 1.0f, 0.0f));
    objectXform = glm::rotate(objectXform, WristTwistAng * 0.018f, glm::vec3(0.0f, -1.0f, 0.0f));
    objectXform = glm::rotate(objectXform, WristAng * 0.018f, glm::vec3(0.0f, 0.0f, -1.0f));
    objectXform = glm::translate(objectXform, glm::vec3(0.0f, -0.5f, 0.0f));
    objectXform = glm::rotate(objectXform, ElbowAng * 0.018f, glm::vec3(0.0f, 0.0f, -1.0f));
    ...
    //스페이스바를 다시 누르면 teapot은 그 위치에서 움직이지 않음
    glm::mat4 objectXform2 = objectXform;
    if (SpaceBar == 0) {
        DrawObject(objectXform2);
    }
    ...
    //Fingertip1 그리기
    //Finger2가 wrist에 대한 상대적 위치로 그려져야 하므로 fingertip1은 새로운 model2를 정의하여 기존 model의 값이 변하지 않도록 함
    ...
    //Finger2 그리기
    //Fingertip1이 model2를 사용하였으므로 Finger1을 그렸던 model값을 그대로 사용함
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
    ...
    if (LeftButtonDown)
    {
        switch (RobotControl)
        {
            ...
            //Spacebar가 눌렸으면 로봇팔과 같은 크기 만큼 teapot의 변수를 증감해 함께 움직이도록 함
        case 0: BaseTransX += xoffset; BaseTransZ -= yoffset; 
            if (SpaceBar == 1) { 
                TeapotTransX += xoffset; 
                TeapotTransZ -= yoffset; 
            }
            break;
        case 1: BaseSpin += xoffset * 180; 
            if (SpaceBar == 1) { 
                TeapotBaseSpin += xoffset * 180; 
            } 
            break;
        case 2: ShoulderAng += yoffset * -90; ElbowAng += xoffset * 90; 
            if (SpaceBar == 1) { 
                TeapotShoulderAng += yoffset * -90; 
                TeapotElbowAng += xoffset * 90; 
            }
            break;
        case 3: WristAng += yoffset * -180; WristTwistAng += xoffset * 180; 
            if (SpaceBar == 1) { 
                TeapotWristAng += yoffset * -180; 
                TeapotWristTwistAng += xoffset * 180; 
            }
            break;
