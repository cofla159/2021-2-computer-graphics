...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		PhongShader->setFloat("lightcons", 1.0f);
		PhongShader->setFloat("lightlin", 0.09f);
		PhongShader->setFloat("lightquad", 0.032f);
		...
		FloorShader->setInt("spot", spot);
		FloorShader->setFloat("lightcons", 1.0f);
		FloorShader->setFloat("lightlin", 0.09f);
		FloorShader->setFloat("lightquad", 0.032f);

		std::cout << (spot ? "spot" : "point") << std::endl;
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS)
		camera.ProcessKeyboard(FORWARD, 30 * deltaTime);
